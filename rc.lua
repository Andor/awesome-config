...
-- mybattery:connect_signal("mouse::enter", function() end)
-- mybattery:connect_signal("mouse::leave", function() end)

-- загрузка индикатора батареи
battery = require("lib.hw.battery")
-- виджет индикатора заряда батареи
mybattery = text({ align = "left", timeout = 10, callback = get_battery_text })
modkey = "Mod4"
...
-- {{{ Tags
tags = {}
tags[1] = awful.tag({ "e", "t", "b", "1", "2"}, 1, layouts[1])
for s = 2, screen.count() do
   tags[i] = awful.tag({ "all" }, s, layouts[1])
end
-- }}}
...
mytextclock = awful.widget.textclock(" %Y.%m.%d, %A, %T ", 1)
